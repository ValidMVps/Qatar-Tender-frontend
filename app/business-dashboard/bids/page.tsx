...
export default function MyBidsPage() {
  ...
  const handleResubmitBid = async (bidId: string) => {
    ...
    try {
      const result = await resubmitRevisedBid(bidId);

      if (result.success) {
        // Update bid status to "submitted"
        setBids((prev) =>
          prev.map((bid) =>
            bid.id === bidId ? { ...bid, status: "submitted" } : bid
          )
        );

        toast({
          title: t("bid_resubmitted_successfully") || "Bid resubmitted",
          description:
            t("bid_has_been_resubmitted") ||
            "Your bid has been resubmitted successfully",
        });
      ...
      <div className="container mx-auto px-6 py-8 space-y-8 bg-gray-50 min-h-screen">
                    ...
                    <TableCell className="px-6 py-5 text-right">
                      <DropdownMenu>
                        ...
                        >
                          ...
                          {/* Show resubmit option only for revision bids */}
        ...
        {selectedBidForDetails && (
          <Dialog
            ...
            onOpenChange={setShowBidDetailsModal}
          >
            <DialogContent className="sm:max-w-6xl max-h-[90vh] overflow-y-auto p-0 border-0 shadow-2xl rounded-3xl bg-white backdrop-blur-xl animate-in zoom-in-95 duration-200">
              <div className="relative">
                <DialogHeader className="p-8 pb-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-t-3xl">
                  <button
                    onClick={() => setShowBidDetailsModal(false)}
                ...
                <div className="p-8 space-y-8">
                  ...
                  <DialogFooter className="flex flex-wrap gap-3 p-8 pt-0 border-t border-gray-100">
                    ...
                    {/* Show resubmit button only for revision bids */}
                    {selectedBidForDetails.status ===
                      "returned_for_revision" && (
                      <Button
                        onClick={() =>
                          handleResubmitBid(selectedBidForDetails.id)
                        }
