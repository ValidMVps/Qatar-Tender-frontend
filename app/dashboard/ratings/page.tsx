...
import { Star, Search, TrendingUp, MessageSquare, Award, Clock, Edit, Save, X } from "lucide-react"
...
const mockReviews: Review[] = [
  ...
  },
  {
    ...
    rating: 4,
    comment: "Professional and organized project owner. Good communication throughout the project lifecycle.",
      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          ...
          className={`${sizeClasses[size]} ${
            star <= rating ? "fill-gray-900 text-gray-900" : "fill-gray-200 text-gray-200"
          }`}
        />
      ))}
    </div>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <Avatar className="w-12 h-12">
            <AvatarImage src={review.contractorAvatar || "/placeholder.svg"} alt={review.contractorName} />
          ...
          <div>
            <h3 className="font-semibold text-gray-900">{review.contractorName}</h3>
    ...
    <CardContent>
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-sm font-medium text-gray-900 mt-1">{review.projectName}</p>
        </div>
      </div>
    </CardContent>
  </Card>
    ...
    <Card className="border-gray-200 bg-white">
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <Avatar className="w-12 h-12">
              <AvatarImage src={review.projectOwnerAvatar || "/placeholder.svg"} alt={review.projectOwnerName} />
              <AvatarFallback className="bg-gray-100 text-gray-600">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">{review.projectOwnerName}</h3>
              <div className="flex items-center gap-2 mt-1">
                {isEditing ? (
                  <EditableStarRating
                ...
                <span className="text-sm text-gray-500">{isEditing ? editedReview.rating : review.rating}/5</span>
              </div>
            </div>
          </div>
...
const ReviewsRatingsPage: React.FC = () => {
  ...
  const [reviewsGiven, setReviewsGiven] = useState<ReviewGiven[]>(mockReviewsGiven)
  ...
  const filteredReviewsGiven = useMemo(() => {
    return reviewsGiven.filter((review) => {
      const matchesSearch =
        review.projectOwnerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        review.projectName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        review.comment.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const handleReviewUpdate = (updatedReview: ReviewGiven) => {
    setReviewsGiven((prev) => prev.map((review) => (review.id === updatedReview.id ? updatedReview : review)))
  }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {filteredReviewsGiven.length > 0 ? (
                filteredReviewsGiven.map((review) => (
                  <EditableReviewCard key={review.id} review={review} onUpdate={handleReviewUpdate} />
                ))
              ) : (
