  ...
  const renderStars = (rating: number) => {
    ...
    return (
      <div className="flex items-center">
        ...
        {hasHalfStar && <Star className="h-3 w-3 fill-yellow-500 text-yellow-500 opacity-50" />}
        ...
        <CardContent className="p-6">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
              ...
              <h3 className="font-semibold text-gray-900 text-lg mb-2">{tender.title}</h3>
              ...
              <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                {tender.jobType === "Hourly" && (
                  <>
                    <span className="font-medium">{tender.hourlyRate}</span>
                    {tender.estimatedTime && <span>Est. time: {tender.estimatedTime}</span>}
                    {tender.hoursPerWeek && <span>{tender.hoursPerWeek}</span>}
                  </>
                )}
                {tender.jobType === "Fixed-Price" && (
                  <span className="font-medium">Fixed price - Est. budget: {tender.budget}</span>
                )}
              </div>
              <p className="text-sm text-gray-700 line-clamp-3 mb-3">{tender.description}</p>
              <div className="flex flex-wrap gap-2 mb-3">
                <Badge variant="secondary" className="bg-emerald-100 text-emerald-800 border-emerald-200">
                  {tender.category}
                </Badge>
              </div>
              <div className="text-sm text-gray-600 mb-4">Proposals: {tender.proposals}</div>
              <Link href={`/business-dashboard/service-providing/tender-details/${tender.id}`} passHref>
