...
// 🔁 Mock tender data to simulate "edit"
const mockExistingTender = {
  title: "Office Renovation in Lusail",
  description: "Renovation of corporate office space in Lusail, Qatar.",
  category: "construction",
  budget: "750000",
  location: "Lusail, Qatar",
  contactEmail: "admin@qatarsolutions.qa",
};
...
const EditTenderModal = ({
  ...
  useEffect(() => {
    if (open) {
      setTimeout(() => {
        setFormData({ ...mockExistingTender, attachments: null });
        setLoading(false);
      }, 500);
    }
  }, [open]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Updated tender data:", Object.fromEntries(payload.entries()));
    router.push("/tenders");
  };
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl dark:bg-gray-900/70 border-none">
          <p className="p-4 text-muted-foreground">Loading tender...</p>
        </DialogContent>
      </Dialog>
    );
  }
      ...
      <DialogContent className="max-w-4xl dark:bg-gray-900/70 border-none">
        <div className="">
          <Card className="w-full border-none shadow-none bg-transparent">
            <CardHeader>
              <CardTitle className="text-3xl font-bold">Edit Tender</CardTitle>
              <CardDescription>
            ...
            <form onSubmit={handleSubmit}>
              <CardContent className="grid gap-6">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="attachments">Replace Image (Optional)</Label>
                  <Input
                    id="attachments"
                    type="file"
